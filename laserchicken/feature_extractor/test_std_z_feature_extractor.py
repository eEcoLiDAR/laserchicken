...
from laserchicken.feature_extractor.std_z_feature_extractor import StdZFeatureExtractor


class TestStdZFeatureExtractor(unittest.TestCase):
    def test_height_stats(self):
        ...
        std_z = StdZFeatureExtractor().extract(pc_in, neighborhood, None, None, None)
        np.testing.assert_allclose(std_z, 1.3567741153191268)

    def test_height_stats_without_neighbors(self):
        pc_in = read_las.read("testdata/AHN2.las")
        neighborhood = []
        std_z = StdZFeatureExtractor().extract(pc_in, neighborhood, pc_in, None, None)
