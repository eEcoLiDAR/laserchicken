...
class TestVariationZFeatureExtractor(unittest.TestCase):
    def test_height_stats(self):
        ...
        std_z = VariationZFeatureExtractor().extract(pc_in, neighborhood, None, None, None)
        np.testing.assert_allclose(std_z, 1.8408359999999995)

    def test_height_stats_without_neighbors(self):
        pc_in = read_las.read("testdata/AHN2.las")
        neighborhood = []
        std_z = VariationZFeatureExtractor().extract(pc_in, neighborhood, pc_in, None, None)
